...
let insert_contact user_id contact_id fav () = let open Contacts in
  Lwt.bind (Db.exec (add_contact user_id contact_id fav) ()) (fun result ->
    match result with
    | Ok data -> Lwt.return (Ok data)
    | Error error -> Lwt.fail (failwith (Caqti_error.show error)))

let get_contacts_from_userid user_id () = let open Contacts in 
  Lwt.bind (Db.find (get_contacts user_id) ())(fun result ->
    match result with
    | Ok data -> Lwt.return (Ok data)
    | Error error -> Lwt.fail (failwith (Caqti_error.show error)))

let get_favorites user_id () = let open Contacts in 
  Lwt.bind (Db.find (view_favorites user_id) ())(fun result ->
    match result with
    | Ok data -> Lwt.return (Ok data)
    | Error error -> Lwt.fail (failwith (Caqti_error.show error)))

let update_make_favorite user_id contact () = let open Contacts in 
  Lwt.bind (Db.exec (make_favorite user_id contact) ())(fun result ->
    match result with
    | Ok data1 -> Lwt.bind (Db.find (view_favorites user_id) ())(fun result ->
      match result with
...
let update_remove_favorite user_id contact () = let open Contacts in 
  Lwt.bind (Db.exec (remove_favorite user_id contact) ())(fun result ->
    match result with
    | Ok data1 -> Lwt.bind (Db.find (view_favorites user_id) ())(fun result ->
      match result with
      | Ok data2 -> Lwt.return (Ok data2)
