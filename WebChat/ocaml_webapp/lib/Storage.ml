...
MESSAGES. THATS WHEN WE USE AN ASSOCIATIVE TABLE BUT ILL LOOK INTO THAT*)
  ...
  let add_msg_sql senderid convoid msg = unit ->. unit @@ 
  "INSERT INTO msglst (senderid, convoid, msg) VALUES ('" ^ senderid ^ "', '" ^ convoid ^ "', '" ^ msg ^ "')"
  ...
  let read_msg_of_recipient_sql = string ->* (Caqti_type.custom ~encode:(fun ({convoid; msg} : recipient_message) -> 
    Ok (convoid, msg)) 
    ~decode:(fun (convoid, msg) -> Ok {convoid; msg}) Caqti_type.(tup2 int string)) @@ 
    "SELECT * FROM msglst WHERE convoid = ?"

    (*
  let read_conversation_sql = string ->* (Caqti_type.custom ~encode:(fun ({senderid; convoid; msg} : message)-> 
    Ok (senderid, convoid, msg)) 
    ~decode:(fun (senderid, convoid, msg) -> Ok {senderid; convoid; msg}) Caqti_type.(tup3 int int string)) @@ 
    "SELECT * FROM msglst WHERE senderid = ? AND convoid = ?"
    *)
  let read_all_sql = unit ->* (Caqti_type.custom ~encode:(fun ({senderid; convoid; msg} : message)-> 
    Ok (senderid, convoid, msg)) 
    ~decode:(fun (senderid, convoid, msg) -> Ok {senderid; convoid; msg}) Caqti_type.(tup3 string string string)) @@ 
...
let add_msg senderid convoid msg () = let open Message in
  Lwt.bind (Db.exec (add_msg_sql senderid convoid msg) ()) (fun result ->
  match result with
  | Ok data -> Lwt.return (Ok data)
  | Error error -> Lwt.fail (failwith (Caqti_error.show error)))
...
let read_recieved_msgs convoid () = let open Message in
  Lwt.bind (Db.iter_s (read_msg_of_recipient_sql) (fun data -> Lwt_io.print (data.msg^"\n") >>= 
  Lwt.return_ok) convoid) (fun result ->
  match result with
  | Ok data -> Lwt.return (Ok data)
  | Error error -> failwith (Caqti_error.show error))
...
(* TODO 
let read_parsed_convo_msgs senderid convoid () = let open Message in
  let sender = read_sent_msgs senderid in
    let recipient = read_received_msgs convoid in
      sender @@ recipient (this is how it should work in theory) 
*)

let read_all () = let open Message in
  Lwt.bind (Db.fold (read_all_sql) (fun {senderid; convoid; msg} acc -> 
  {senderid; convoid; msg} :: acc) () []) (fun result ->
  match result with
  | Ok data -> Lwt.return (Ok data)
  | Error error -> failwith (Caqti_error.show error))
