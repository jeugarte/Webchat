...
(** [get_conversations] returns the conversations of a specfic user RI:
    takes in a user id*)
 let get_conversations = let open UserConversation in let open User in
   App.post "/getConversations" (fun request -> Lwt.bind (Request.to_json_exn request)
   (fun contact_json -> let users_info = contact_json |>
   usercontact_of_yojson in Lwt.bind(id_from_email users_info.email ())(fun response -> match response with
    | Ok userid -> Lwt.bind (read_conversations_given_user userid ()) (fun convoids -> match
...
let get_contacts = let open User in let open Contacts in App.post
   "/getContacts" (fun request -> Lwt.bind (Request.to_json_exn request)
   (fun contact_json -> let users_info = contact_json |>
   usercontact_of_yojson in Lwt.bind(id_from_email users_info.email ())(fun response -> match response with
    | Ok userid -> Lwt.bind (read_contacts_given_userid userid
...
let make_favorite = let open User in let open Contacts in App.post
   "/makeFavorite" (fun request -> Lwt.bind (Request.to_json_exn request)
   (fun contact_json -> let both_info = contact_json |>
   both_of_yojson in Lwt.bind(id_from_email both_info.user_email ())(fun user_response -> match user_response with
    | Ok userid -> Lwt.bind(id_from_email both_info.contact_email ()) (fun contact_response -> match contact_response with
      | Ok contactid -> Lwt.bind (does_contact_exist userid contactid
   ()) (fun response -> match response with 
          | Ok true -> Lwt.bind
              (update_make_favorite userid contactid ()) (fun update -> match update
                with 
            | Ok _ -> Lwt.return (Response.of_plain_text "Success") 
            | Error e -> Lwt.fail (failwith e)) 
          | Ok false -> failwith "contact does notexist" 
          | Error e -> Lwt.fail (failwith e))
      | Error e1 -> Lwt.fail (failwith e1))
    | Error e2 -> Lwt.fail (failwith e2))))

let add_contact = let open Contacts in let open User in App.post
   "/addContact" (fun request -> Lwt.bind (Request.to_json_exn request)
   (fun contact_json -> let both_info = contact_json |>
   both_of_yojson in Lwt.bind (id_from_email both_info.user_email ())
   (fun accepted -> match accepted with 
   | Ok user_id -> Lwt.bind(id_from_email both_info.contact_email ()) (fun contact_response -> match contact_response with
   | Ok contactid -> Lwt.bind
      (does_contact_exist user_id contactid ()) (fun existing_responses ->
      match existing_responses with 
        | Ok true -> Lwt.return
        (Response.of_plain_text "Contact already exists") 
        | Ok false ->
          Lwt.bind (insert_contact user_id contactid false ()) (fun add -> match
          add with 
            | Ok _ -> Lwt.return (Response.of_plain_text "Success") 
            | Error e -> Lwt.fail (failwith e)) 
